    ...
    for (var i in data) {
        ...
        db.insertNode({
            ...
            for (var sf in shop.facilities) {
                db.insertRelationship(node._id, 6, 'HAS', { title: '7 days'});
                db.insertRelationship(node._id, 10, 'HAS', { title: 'atm'});
                for (var f in facilitiesD) {
                    // if (sf == f) {
                    //     db.insertRelationship(node._id, 2330, 'HAS');
                    // }
                }
            }
            counter += 1;
